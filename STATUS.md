...
- **咖啡廳第三層研發(本批,🔴 會動平衡)**:名店期缺口 100% 是客單價,本批只動那條腿
  (客流與成本結構一格未動)。① 修掉三項**數學上精確 0** 的死研發(冷萃/磅蛋糕/司康的毛利
  恰好等於基礎菜單加權毛利 $19.00)⇒ 調價 $41/$43/$41;② 補第三層三項 $64/$58/$62,
  前置是**招牌 Lv2/Lv3/Lv4**(研發費固定 $3,000/6 天,門檻由招牌承擔);③ 第七種原料
  **精品生豆**($14 乾貨,每客用料只從 3.90 動到 3.91)。
  實測(`cafe-growth-sim.ts`【C】):開張 **+$96**(零位移)、成長 **+$330 → +$431**(佔淨租金
  30% → **40%**)、成熟 **+$586 → +$765**(54% → **71%**)、名店 **+$917 → +$1,244**(85% →
  **115%**)、全設備 **+$1,355 → +$1,693**(125% → **156%**)。其中**純調價**就佔 85% → 92%。
  逐項回本 48.8/58.0/41.8 天(門檻 60);菜單均價 $38.15 → $43.13,夾值 $55 仍未生效。
  **兩條既有護欄前提被推翻、已改寫**(理由與替代品見設計文件 §10.5)。
- **成長曲線重量測(`a1e0efc`,只量不改)**:08-16 分區批次漏掉量測腳本 `cafe-growth-sim.ts`
  ⇒ 實測表過期三週;補齊擺設後量出上面那組基準並歸因缺口 100% 在客單價。
- **咖啡廳可見性批次(`4aa6e8b`)**:診斷是**資訊缺口不是數值失衡**;TODAY 三格、binding 建議、7 日流失摘要、投資 chip。**純可見性、零漂移**。
- **本機有四批未 push**(`3f6140f`、`4aa6e8b`、`a1e0efc`、本批),`origin/main` 仍在 `dd805bd`。
- 本機驗證:`npm test` **115/115**、typecheck、`build` 全綠、`balance-test` **零漂移未 `--update`**
  (快照局從不開張咖啡廳)、`smoke:save` **49/0**;`SAVE_VERSION` 仍 10(新研發 id 是 additive
  字串陣列、新原料缺 key ⇒ `safeUnits()` 夾 0)

## 近期已部署基線

- **可見性批次(08-25,未 push)**:`cafe.ts` 五支純函式 + `CafeSalesDay.turnedAway` 選填欄位;投資文案用**差分**再呼叫一次真公式 ⇒ 不可能漂開(設計文件 §9)。
...
- 🔴 **實玩觀察(併成同一輪、跑十幾個遊戲日、用舊存檔開局)**:① **AI 當日觀察是否恢復**——Console
  跑 `rentDebug.narrateStatus()` 讀 `lastExit.reason`;② 新互動與雙人動作觀感;③ 打架門檻 50/22;
