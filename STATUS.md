...
- **咖啡廳顧客互卡修復(本批,未 push)**:使用者回報「客人被椅子卡住／被等候先卡住」——
  同一個根因:`detourGrid()` 把**移動中的人**排除在障礙外,而卡死的人 `moving` 永遠是 `true`
  ⇒ 互卡的兩人在彼此的繞路地圖上互相隱形(實測 3000 秒不解開)。四層修法:
  ① `stallT` 分辨「通過中/原地打轉」+ 正面對撞依 **id 字典序**側身讓路(零 RNG);
  ② `seated`／`leaving` 補脫困逾時(站著用餐/原地淡出,**不再憑空消失**);
  ③ 人龍避開座位出入口且不排進後場;④ 開張贈品重排成沒有死巷席位。
- **驗收數字**:情境 B(8 內用+3 外帶/5 店員)未達目標 **7 → 0**;門檻掃描 5~9 內用 × 0~4 外帶
  × 店員 1/3/5/∞ 共 100 格 **44 格卡死 → 0**;門檻以下的通行反而更快(33.05s → 26.60s)。
- **補上系統性缺口**:`sim-trace.ts` 以前一行都沒跑到 `guestAgents.ts`;現在整批進場情境與
  席位死巷/人龍幾何檢查都進了 CI(共用 `scripts/cafe-crowd-sim.ts`)。
- 驗證:`npm test` **115/115**、typecheck、`build`、`smoke:save` **49/0** 全綠;
  `balance-test` **零漂移未 `--update`**。無 `.vue` 變更,`SAVE_VERSION` 維持 10。
- ⚠️ 第 4 層只影響**新開的局**;既有存檔的家具位置不變,靠第 1~3 層改善(已實測:舊擺法
  在新程式下情境 B 與整份掃描同樣零卡死)。
