...
- **咖啡廳分區規則進 UI(本機完成,尚未 push)**:商店標籤／家具詳情／CafePanel「分區小抄」
  三個 UI 落點,共讀單一真相來源 `src/content/cafeZoneGuide.ts`。純 UI/文案,不動模擬邏輯。
- **與 `origin/main` 同步(`dd805bd`)**,三角關係(吃醋)系統已部署上線並完成 production 部署驗證
  (bundle `index-KAOr6hB_.js`,線上逐項驗到玩家可見中文文案;細節見下節「近期已部署基線」)。
- 本機驗證:`npm test` **114/114**、typecheck(app+worker)、`build` 全綠、
  `balance-test` **零漂移未 `--update`**、`smoke:save` **49/0**;`SAVE_VERSION` 仍 10

## 近期已部署基線

- **分區規則進 UI(2026-08-23,本機完成,未 push)**:三個 UI 落點共讀 `cafeZoneGuide.ts`;
...
- **咖啡廳掛機節奏是否加回主動性**(已**實質回應**:分區規則進了 UI 三個落點、常客給了跨日可追的對象;是否結案待拍板)
