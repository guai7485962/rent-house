...
- 🔴 **「買了沒作用」的永久投資修好了(未 push;使用者拍板「先修投資,不是整體加收入」)**:根因是
  ...
  **150~253 天**。🔴「改成增加席次」**已被實測否決**(席次腿從來不是 binding)。② **大型冷藏 $15,000 →
  $8,000**,判準改用「備貨過量情境」(名店期 ×1.5 ⇒ **50.1 天**;照建議量 439.6 天而**那是正確的**),
  文案補冷萃閘門、tone `blocked`→`note`。③ **貓跳台一個數值都沒動**,只加送養結果行 + Vue 層分組。
  驗證:`npm test` **116/116**、typecheck、build、`smoke:save` **49/49**、`balance-test` **零漂移未
  `--update`**、`SAVE_VERSION` 不動;UI Lab 18 + 面板自訂 12 張(唯一 404 是容器沒 Worker)。護欄
  **$498 / $51,000 / 102 天**;兩欄:名店期預設 **+$1,464 / 135% 未動**、全設備 156% → **167%**。
...
- 🟠 **咖啡廳成長基準對帳(未 push;只動文件與量測腳本,`src/` 一行未動)**:§4.7 的客單價階梯
  $36/$41/$47/$53 是**未加權**顯示值、淨利卻是 `baseWeight` **加權**算 ⇒ 蘋果對橘子,而回歸全綠是因為
  **壞掉的是設計表不是程式**;665 種合法組合上限只有 $44.91/$43.17 ⇒ **$47/$53 做不到**,
  四階段目標重訂 **+$124/+$398/+$765/+$1,051**(客流欄未動),量測基準改以回本天數納入設備。
  驗證:`npm test` 116/116、typecheck、build、`balance-test` 零漂移;沒動 `src/` ⇒ 免 `smoke:save`。
