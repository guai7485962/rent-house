...
- **咖啡廳互動密度(本批,未 push)**:使用者要「多一點房客互動」;方向是**讓現有功能真的會
  發生**,不加新功能。「縮短下樓週期」數學上做不到同框——舊閘門要兩人同天須 `GAP | 雜湊差`,
  而種子局差值 `394,033,487 = 73 × 5,397,719`(皆質數)⇒ 任何合理 GAP 都是 **0%,永遠**。
- **五項改動**:① 聚會 `need` 3 → **2**(種子局只有兩人,原本**永遠觸發不了**);
  ② 閘門加 `CAFE_SIT_WINDOW_DAYS = 2` + 選時段 `hash % len` → **`candidates[0]`**(讓那行
  「依偏好排序」的過期註解成真)——🔴 **缺一不可,任一改回舊值同框率就歸零**;
  ③ `GAP 5 → 4`、`CAFE_FIRST_DAY 14 → 11`(**不得再低**,三個測試檔斷言 `> 10`);
  ④ 補掉**三條**把 `at_cafe` 租客傳送回樓上的路徑(`tick` 串門子、`interactions` 分組與
  `thirdPresent`——session tile 在 `agents.ts` 蓋過 `activityTile`);⑤ 不再固定與顧客撞同一格。
- **實測同框率**(對 200 個遊戲日逐日統計):同日同小時 ≥2 人,種子局 **0% → 25%**、
  4 人局 **≈0% → 50%**(六種原型組合皆同);每人下樓間隔 5 → **2** 個遊戲日。
- 驗證:`npm test` **115/115**、typecheck、`build`、`smoke:save` 全綠;`balance-test`
  **零漂移未 `--update`**;無 `.vue`,`SAVE_VERSION` 10。三條後續已進 `docs/待辦.md`。
...
- 🔴 **咖啡廳互動密度實玩驗收**:開新局→開張→快轉到第 11 遊戲日,確認 ① 租客下樓坐 **14:00**、
  ② 同框日兩人真的同時在一樓、③ 那一小時 sprite **沒被拉回樓上**(三條傳送路徑)、④ 不再疊格、
  ⑤ Feed 出現「☕ 打烊後的一樓」。嫌吵就調 `CAFE_SIT_GAP_DAYS`(**勿動 WINDOW**)。
