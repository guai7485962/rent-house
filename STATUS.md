...
- **本地領先 `origin/main` 兩個 commit(咖啡廳 A 批地板分區、B 批常客系統),尚未 push、尚未部署**;
  線上仍是劇情弧多樣性第二／三階段的 `def991b`(bundle `index-sSFUhi4o.js`)
- **B 批(常客系統,§4.11)**:6 位顧客變成跨日認得出來的人。**身分鍵是姓名**;3 個來訪日升格;
  回訪走「每營業小時 18% 抽籤」**刻意不乘客流** ⇒ 開張期與名店期都約 2 位/日,對齊好感寬限 3 日。
  三個效果:老樣子、好感 ≥60 留 $3 小費、好感 ≥80 帶朋友(= 該小時 `abandonCount −1`,
  **不是多生一位客人**,避免突破產能上限)。姓名池 32→64(append-only)
- **A 批(地板分區,§4.10)**:「家具擺在對的區才有機能效果,擺錯區只剩氛圍」——後場→庫存容量、
  寵物區→寵物停留與認養詢問、吧台區→同時服務人數。**⚠️ 唯一會咬既有存檔的改動**(已核可):
  雇 4 人以上且吧台仍是贈品那座 ⇒ 產能 104→78,面板有提示
- **界線未破**:`cafeAmbiancePoints()` 一行未改(客流乘數零漂移);`cafe.ts` 仍不 import
  `placements`。**安全底線**:常客不進 `state.runtimes`、不建 `relationships`、不參與戀愛線
  (寫在 `sim/cafe.ts` 第 16 節檔頭,`cafe-regular-test.ts` 有掃原始碼的斷言)
- **最新驗證(全綠)**:`npm test` **106/106**(新增 `cafe-regular-test.ts` 109 條)、
  app + worker typecheck、`npm run build` 通過、`balance-test` **零漂移**(未 `--update`);
  `npm run ui:shot -- rent` 18 張截圖 0 error / 0 warning,另跑自訂常客區塊流程 6 張 0 error
- **存檔版本**:`SAVE_VERSION = 10` 不變;B 批的三個新欄位(`cafe.regulars` /
  `regularCandidates` / `regularCandidateDays`)是 additive,舊檔補空值、**不回填假資料**
...
- **中控驗收 A／B 兩批的 diff 後再決定 push 與部署時機**(本地已 commit ×2,未 push)
- **實玩觀察常客系統**:跑十幾個遊戲日確認第一位常客約第 4~5 天出現、回訪不會太密或太疏、好感不亂掉
