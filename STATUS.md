...
- 🟠 **咖啡廳成長基準對帳(未 push;只動文件與量測腳本,`src/` 一行未動)**:設計表 §4.7 的客單價
  階梯 $36/$41/$47/$53 與 `avgTicket()` 是**同一個量**——**未加權**、給玩家看的顯示值;但淨利是照
  `baseWeight` 50/30/20 **加權**算的 ⇒ 拿前者訂目標、拿後者對帳,從第一天起就是蘋果對橘子,
  而回歸測試一路全綠是因為**壞掉的是設計表,不是程式**。掃描 665 種**前置合法**的研發組合:未加權
  上限 **$44.91**、加權上限 **$43.17** ⇒ **$47 / $53 任何組合都做不到**。四階段目標重訂為 **+$124 /
  +$398 / +$765 / +$1,051**(客流欄**一格未動**:實測 24.5/49.4/74.8/118.3 對設計 26/48/75/110)。
  🔴 **目標下修不代表玩家收入變多 —— 一個遊戲數值都沒動**;「要不要加收入」是使用者拍板順序的
  第二步,**尚未發生**(`docs/待辦.md` 新開一條)。
- **量測基準納入設備**:`cafe-growth-sim.ts` 新增「客單價階梯 / 天花板掃描 / 階段目標重算 / 設備逐項
  回本 /【E】含設備基準」五段。判準是**回本天數**(§10.5 的 60~250 天帶):**只有第二台咖啡機過關**
  (成長 96.6 / 成熟 50.3 / 名店 82.0 天;開張期 243.5 天但買不起、且該列是平衡錨 ⇒ 不納入)。
