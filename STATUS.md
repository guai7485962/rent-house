...
- **咖啡廳可見性批次(本機完成,尚未 push)**:使用者抱怨「投資與研發都沒效果、家具可有可無」,
  診斷是**資訊缺口不是數值失衡**(店被服務位=1 卡死 ⇒ 招牌/戶外/超額氛圍的邊際效益數學上精確為
  0,每天約 64 位客人無聲蒸發)。本批把落差攤到 UI:TODAY 三格(想上門/做得出來/沒接到)、binding
  五選一建議、7 日流失摘要、氛圍上限提示、每項投資的「現在買會怎樣」chip。**純可見性、零漂移**。
- **本機有兩批未 push**(分區規則進 UI `3f6140f` + 本批),`origin/main` 仍在 `dd805bd`。
- 本機驗證:`npm test` **115/115**、typecheck(app+worker)、`build` 全綠、
  `balance-test` **零漂移未 `--update`**、`smoke:save` **49/0**;`SAVE_VERSION` 仍 10

## 近期已部署基線

- **可見性批次(2026-08-25,未 push)**:`cafe.ts` 五支純函式(`cafeTypicalBase`/`cafeAmbianceFull`/
  `cafeBottleneck`/`cafeBottleneckAdvice`/`cafeInvestOutlook`)+ `CafeSalesDay.turnedAway` 選填欄位。
  主數字用「一般日」;投資文案用**差分**再呼叫一次真公式 ⇒ 不可能漂開。論證見設計文件 §9。
- **分區規則進 UI(2026-08-23,未 push)**:三個 UI 落點共讀 `cafeZoneGuide.ts`;
  8 件有機能需求的家具標「該擺哪」,4 件純氛圍家具不標;家具詳情即時比對目前分區給 ok/warn 提示。
- **G 批 互動目錄 18 → 28**:不稀釋的主池/次池抽籤(零新 RNG)、租客雙人繪製管線(4 pose + 3 fx);
  日常/友誼 6 + 戀愛線 4(全 `pool:"extra"`、不進 AI 白名單;🔴 安全靠 `tier` 的 `rel.romantic`
  ——只可能經 `canBecomeCouple()` 建立,四種再一致掛 `both_adult`)
- **H 批 AI 日記卡死修復**:`narrateDay()` 45s 逾時 + `deferredRun` 8 分鐘 watchdog + 四個靜默退出改留標籤 + `safeNarrateCtx()` 降級 + `narrateStatus()` 唯讀診斷
- **I 批 白畫面根因修復**(`2689311`):部署刪舊雜湊資產 + iOS standalone 抓著快取的舊 `index.html` ⇒ 404;修法(`no-store` + 自救腳本)見 `docs/系統總覽.md` 地雷紀錄
...
- **三角關係(吃醋)**:`unrequitedSuitors()` 零新持久化狀態即時推導,安全性繼承既有 `canRomance()`;落選者一次性反應(固定文案 + 既有 heartbreak fx、零新 RNG)
- **更早**:咖啡廳 P1～P4b(逐客結帳、排隊/店員、庫存/研發/成長曲線、收支分頁與可收合面板)、店貓辣椒、月度事件鏈 3→8、姓名池 20→72、職業 15→24、職業目標 8→14、本地劇情種子 10→16 條支線

## 下一步

- 🔴 **實玩觀察(併成同一輪、跑十幾個遊戲日、用舊存檔開局)**:① **AI 當日觀察是否恢復**——遊戲頁
  Console 跑 `rentDebug.narrateStatus()` 讀 `lastExit.reason`(`timeout`／`upstream` ⇒ worker 端、
  `quota` ⇒ 共用金鑰額度、`budget` ⇒ `DEFERRED_DAILY_BUDGET = 6` 太低);② 新互動與雙人動作觀感;
  ③ 打架門檻 50/22 與並肩率 90.6% 的手感;④ ⚠️ **舊存檔的老情侶會補演一次初吻,是已知行為不是 bug**
  (見 `docs/設計檢討與優化.md` §10-2b)。同輪順帶看 A 批產能 nerf(104→78)、常客升格節奏、劇情弧
  多樣性、C 批聚會頻率(`CAFE_GATHER_CHANCE`)、D 批會不會「蓋台」
- 🔴 **可見性批次實玩觀察**:① 三格與 binding 那句是否真的讓玩家知道下一步該買什麼;
  ② 🥐 流失日誌會不會吵(嫌吵就砍它,面板四個落點不依賴它);③ 投資 chip 在邊界翻面的手感
- 🔴 **三角關係實玩觀察重點**:① 落選者固定文案會不會顯得重複(刻意取捨,換零漂移);② AI 會不會
  自己從既有「與 X 曖昧」背景資料寫出單戀情節;③ 只在「B 選定對象」那刻反應一次,無持續性吃醋
- 長線衝突項:壓力門檻改成相對各自基準線(`stress >= baselines(rt).stress - 10`),要先把 `baselines()`
  抽出共用模組解掉循環 import
- 其餘可選項(依 `docs/待辦.md`,🔴 一律先問使用者):牛奶／寵物鮮食建議常備量 24 對開張期菜單過高
  (跟著菜單走**會動到平衡**);第三層研發缺高價品項 ⇒ 名店期客單價卡在 ~$37
  (`CAFE_MAX_AVG_TICKET` 已放寬到 $55);認養卡下拉空白列(🟢 小,空 `v-model`,做法見 `796643f`)

## 待使用者決策(不要自行動工)

- **「租客撿到寵物」要不要再補規則事件**(已**部分解決**,優先度低):`adopt_cat` 只由 AI 事件選項
  帶進來 ⇒ 離線/模板 fallback 時永不發生;月度事件鏈 `stray_litter` 已給了一條不靠 AI 的領養路徑。
  再加規則事件會改變 `rollEvent()` 比對序 ⇒ 可能要重建基準。
...
- **咖啡廳掛機節奏是否加回主動性**(已**實質回應**:分區規則進 UI、常客可跨日追、可見性批次把
  瓶頸與每項投資的邊際效益攤開;是否結案待拍板)
...
| 咖啡廳怎麼設計的 | `docs/一樓寵物咖啡廳-設計.md`(第一版)/ `docs/咖啡廳經營玩法-重設計.md`(**現行**;§9 = 可見性批次) |
