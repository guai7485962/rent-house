...
- **規則事件復活(`7d09071`,已 push = 目前 `origin/main`,線上已驗證)**:`rollEvent()` 的四則根事件(`breakdown`/`sick`/
  `dissatisfied`/`grievance`)實測**從來沒觸發過**,三則續集是 100% 死碼。病根**不是門檻太高**——
  遊戲裡沒有任何「房東虧待房客」會**持續**推動數值的管道:所有懲罰都是脈衝、四條數值全部有回歸
  ⇒ 脈衝一定被抹平(壞著沒修的 `satisfaction −7/日` 實際位移只有 **−1.45**,`affinity` 那側
  **根本沒接線**,而「及時修繕 +5」是**永久**的)。虧待免費、照顧單向加分。
- **改法**:`maintenance.ts` 新增 `neglectPoints(rt)` 0~6(故障拖延天數 + 空頭支票旗標,
  **零新存檔欄位**),掛上**倍率完全不同**的四條管道:stress 掛 `baselines()`(×1)、wellbeing 掛
  每小時增量(×100)、satisfaction 掛 `target`(×1)、affinity 掛**每日一次**(無回歸)——
  🔴 **絕不可共用一組常數**。另把 `stress>=80 → wb −0.4` 階梯改成 `stress>60` 起的斜坡(80 那點仍
  **恰為** −0.4)。門檻**只動 `breakdown`**(90 → 72 且加 `neglect >= 1`),其餘三則**一個數字都沒動**。
  事件冷卻 2 → **4 日**(刻意長於 AI 的 3 日;兩者共用 `lastEventDay`,更短會**餓死 AI 事件**)。
- **實測**(`scripts/event-freq-sim.ts`,5 種子 × 3 房東行為 × 3 情境 × 60 遊戲日,每租客每 30 日):
  四則根事件 `0 / 0.016 / 0 / 0.673` → **`0.40 / 0.53 / 0.26 / 0.35`**;三則續集 **全部 0 → 都 > 0**;
  照顧型房東四則合計 **0.072**(≤0.5)且退租仍為 **0**;AI 事件機會日 caring `0.995→0.995`、
  absent `0.957→0.844`(**−11.8%**)、最壞的 absent×extreme **−21.3%**(門檻 <25%);
  `neglectPoints` p95 = **6 = 上限**(沒有「第六道天花板」)。
- ⚠️ **唯一未達標**:`dissatisfied` 0.264,低於規格的 1~3。規則事件與 AI 事件共用一個冷卻槽
  ⇒ 規則事件總數上限約 2.8/租客·30日,與指標 1 的下界總和**結構性互斥**(詳見設計文件附錄 §5)。
- 驗證:`npm test` **116/116**、typecheck、build、`smoke:save` 全綠;`balance-test` **已 `--update`**
  (只 5 個浮點欄位動、`decisions` 仍為 0,成因見日誌);無 `.vue`,`SAVE_VERSION` **不動**。

## 近期已部署基線

- **咖啡廳互動密度(`a3e9647`,已 push)**:聚會 `need` 3→2、`CAFE_SIT_WINDOW_DAYS`、
  補掉三條把 `at_cafe` 傳送回樓上的路徑;實測同框率種子局 **0% → 25%**、4 人局 **≈0% → 50%**。
...
- 🔴 **實玩觀察**:① 日記／雙人弧是否全程為房東第三人稱;② 房東抉擇是否都是房東可執行的行動;
  ③ Console 跑 `rentDebug.narrateStatus()`;④ 第三層研發**成熟期仍只有 71%**,缺口已不在菜單價帶
  而在產能與客流 ⇒ 下一批的題目
