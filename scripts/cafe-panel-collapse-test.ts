 ...
 *   ① 十一個區塊都可收合,預設只展開「營運觀察」與「常備量」(2026-08-23 加「分區小抄」第十一塊,
 *      預設收合);
...
const SECTIONS = ["today", "research", "menu", "sales", "stock", "staff", "invest", "regulars", "adoption", "rent", "zones"] as const;
/** 每個標題列按鈕的內容(到第一個 </button> 為止)。 */
const heads = [...panel.matchAll(/<button class="section-head"[\s\S]*?<\/button>/g)].map((m) => m[0]);

// ── ① 收合 ────────────────────────────────────────────────
check("十一個區塊的標題列都是收合開關", heads.length === SECTIONS.length);
...
check(
  ...
  /openSections = reactive\(\{[\s\S]*?\}\)/.test(panel)
    ...
    && ["research", "menu", "sales", "staff", "invest", "regulars", "zones"].every((id) => new RegExp(`${id}: false`).test(panel)),
);
check(
...
check(
  ...
  heads.every((head) => /<span class="(capacity|research-count|ticket|window|blamed-count|count|staff-wage|balance|regular-summary|zone-count)/.test(head)
