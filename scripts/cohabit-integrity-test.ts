/**
 * 同居唯一性回歸：已有同居對象的人可保留其他戀愛關係，但不能用第三人的申請覆蓋住處。
 */
import {
  state,
  resolveCohabit,
...
import { relationships, pairKey } from "../src/sim/social";
...
relationships[pairKey(B, C)] = { value: 96, romantic: true, cohabitOffered: true };
...
check("第三人戀愛關係可以保留", relationships[pairKey(B, C)]?.romantic === true);
check("取消後可在未來重新提出", relationships[pairKey(B, C)]?.cohabitOffered === false);
check("玩家會收到申請取消原因", state.notice.includes("已有同居對象"));

relationships[pairKey(A, C)] = { value: 97, romantic: true, cohabitOffered: true };
