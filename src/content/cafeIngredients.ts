 ...
 * 3. 三個菜單分支(沖煮 / 烘焙 / 寵物餐點)**每一條都至少有一乾一鮮**,
 *    所以沒有任何一條研發線可以完全避開缺貨,也沒有任何一條會被損耗整批掃掉。
 *    (2026-08-25 第三層上線後 brew 分支變成 2 乾 1 鮮:咖啡豆 + 精品生豆 + 牛奶。
 *    規則守的是「每條線都同時碰得到缺貨與損耗」,不是「恰好各一個」。)
...
/**
 * 七種原料。設計文件 §5.2 只舉了四個例子,CAFE-12 增為六(三乾三鮮),
 * 2026-08-25 的第三層研發再 append 第七種**精品生豆**(乾貨)⇒ 四乾三鮮。
 *
 * ## 🔴 第七種原料為什麼要存在(2026-08-25)
 *
 * 第三層三項(季節限定單品豆 / 貓掌造型拿鐵 / 午後茶點套餐)是 $58~64 的高價品,
 * 若沿用既有六種原料達成 45~60% 毛利帶,配方就得堆到 12~16 單位
 * ⇒ 每客平均用料由 3.90 拉到 4.30(+10%),名店期常備量會超過後場上限 920 單位
 * ⇒ 天天缺貨,曲線反而往下掉。改用高單價的精品生豆($14/單位)後,每客用料
 * 3.90 → **3.91**,庫存腿一格不動。
 *
 * 另外三項第三層**全部**吃精品生豆 ⇒「缺精品生豆 = 高價品全線停售」是乾淨可讀的
 * 新失敗模式;而且它是**乾貨**,完全不碰 `SPOILAGE_FREE_UNITS` / `SPOILAGE_RATE`
 * 那組已經校準好的損耗唯一解。
 *
 * ⚠️ 舊存檔沒有這個 key ⇒ `safeUnits()` 夾成 0、常備量顯示 0。**這是正確行為**:
 * 還沒研發第三層就不該進生豆,玩家研發完按「依上週銷量建議」就會補上。
 *
 ...
 * 2 單位麵粉 + 1 單位奶油($16)。低單價換來的整數粒度,才有辦法讓 16 個品項
 * (基礎 3 + 研發 13)的毛利**全部落在 45～60%** 這個 band 裡——
...
export const CAFE_INGREDIENTS = [
  ...
  {
    // 🔴 2026-08-25 append 的第七項。**必須留在陣列最後面**:補貨配額依宣告序,
    // 插到中間等於靜默改變「錢不夠時先補誰」。
    id: "specialty_bean",
    name: "精品生豆",
    unitPrice: 14,
