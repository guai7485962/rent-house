...
export const CAFE_UPGRADES = [
  ...
  },
  {
    ...
    /**
     * 🔴 **$15,000 → $8,000(2026-09-02)。它是保險 + 閘門,不是產能。**
     *
     * 實測:損耗的天花板只有 **$18.2/日**(名店期),而冷藏拿走的是那 100%
     * ($18.2 → $0.0)—— 不是沒接上,是「可影響的量」本來就近乎零。根因在
     * §4.3.1「懶人路線必須零損耗」把 `SPOILAGE_FREE_UNITS = 23` 釘死成唯一解。
     *
     * 它唯一值錢的情境是**玩家自己備太多**(常備量 ×1.5):名店期救回 **$99.5/日**。
     *
     * ⚠️ **純調價無解的證明**:它的價值方差是 $0 ~ $99.5/日(**5.5 倍**),
     * 而 §10.5 的回本帶寬只有 250/60 = **4.17 倍** ⇒ **任何單一售價都不可能
     * 讓兩端同時落在帶內**。所以判準改掉:它用「備貨過量情境」判定。
     * $8,000 ⇒ 備貨過量回本 80 天(落在帶內);玩得好、從不備太多的人是 440 天,
     * 而**那是正確的** —— 保險就是這樣,沒出事就是白買。
     *
