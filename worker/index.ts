...
- 玩家會另外指定性別，gender 必須逐字採用玩家指定值，不得依名字或描述自行猜測或改寫。
...
async function handleInvite(req: Request, env: Env): Promise<Response> {
  ...
  let body: { name?: string; description?: string; gender?: string };
  try {
  ...
  const gender = ["male", "female", "nonbinary"].includes(String(body.gender)) ? String(body.gender) : "";
  if (!name || !description || !gender) return Response.json({ error: "bad_request" }, { status: 400 });

  const user = `特邀租客的名字:${name}\n玩家指定性別:${gender}\n個性描述:${description}\n請輸出角色資料 JSON，gender 必須是 ${gender}。`;
  try {
    const text =
      provider === "gemini"
